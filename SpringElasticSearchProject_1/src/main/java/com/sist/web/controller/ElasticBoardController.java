 ...
 *  		LogStash: 데이터 수집 (DB, SCV) => 집계 , 파싱 => ElasticSearch
...
public class ElasticBoardController {
	...
	public String eboard_list(@RequestParam(name="page",required=false) String page,Model model)
	{
		 ...
		 * findById(int id) : SELECT * FROM board WHERE id-2
	...
	public String board_insert_ok(ElasticBoard vo)
	{
		vo.setHit(0);
		vo.setId(idMaxData());
	...
	public String eboard_detail(@RequestParam("id") int id,Model model)
	{
		...
		bDao.save(vo);//update => 패 (이미 id존재할때는 update)
		vo=bDao.findById(id);
